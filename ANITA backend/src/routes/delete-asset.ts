 ...
 * Deletes an asset from Supabase assets table
 */
...
export async function handleDeleteAsset(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (error) {
      logger.error('Error deleting asset', { error: error.message, requestId, assetId, userId });
      res.status(500).json({
    ...
    const requestId = req.requestId || 'unknown';
    logger.error('Unexpected error in delete-asset', { 
